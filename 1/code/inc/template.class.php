...
class template {
	...
	function gettpl($file) {
		...
		$cachefile =$this->objfile;// 'ucenter/cache/'.basename($this->objfile);	
		$cachecontent = '';
		$mmc = memcache_init();
		if($mmc==false)
		{
	...
	function complie() {
		...
		$template = "<?php if(!defined('APP_PATH')) exit('Access Denied');?>\r\n$template";
		...
		$filename =$this->objfile;// 	'ucenter/cache/'.basename($this->objfile);
		$mmc = memcache_init();
		if($mmc==false)
		{
	   	echo "mc init failed\n";
